...
The `ffcr` functions encompasses three different methods to train fast-and-frugal trees. These are named *basic*, *greedy*, and *cross-entropy*.^[In the book [@katsikopoulos2021], we refer to the cross-entropy optimization [@rubinstein1999] as the *best-fit* method. It does not guarantee to find the best possible tree but often produces more accurate trees than the other simpler methods.]
...
Alternatively, we can call the function using the formula syntax. Here we train the fast-and-frugal tree using only a few selected features.

```{r, eval=FALSE}
fftree(diagnosis ~ age + albumin + proteins + aspartate, data = liver)
```


Printing the model shows the structure of the tree and its fitting performance in the data set. 
